...
export class Arena1v1Room extends Room<ArenaState> {
  ...
  onCreate(): void {
    ...
    this.killVolumes = [];
    console.log(
      `[Arena1v1] Duel arena ready with ${this.spawnPoints.length} spawn points.`
    );
  ...
  private pickSpawn(): { x: number; y: number; z: number } {
    ...
    const fallbacks: [number, number, number][] = [[-7, 0.2, 0], [7, 0.2, 0]];
  ...
  onJoin(client: Client): void {
    ...
    console.log(
      `[Arena1v1] Client ${client.sessionId} joined. Players in room: ${this.state.players.size}`
    );
  }
  ...
  onLeave(client: Client): void {
    ...
    console.log(
      `[Arena1v1] Client ${client.sessionId} left. Players in room: ${this.state.players.size}`
    );
  }
        ...
        if (hitResult) {
          const target = this.state.players.get(hitResult.targetId);
            ...
            if (process.env.DEBUG_HITSCAN) {
              console.log(
                `[Arena1v1] Hit: ${shooterId} -> ${hitResult.targetId} (${hitResult.hitboxType})`
              );
            }
          }
          ...
          console.log(
            `[Arena1v1] Shoot miss: ${shooterId} at (${player.x.toFixed(1)},${player.y.toFixed(1)},${player.z.toFixed(1)}) yaw=${player.yaw.toFixed(2)} pitch=${player.pitch.toFixed(2)} (${otherCount} other players)`
          );
        }
      }
    });
  ...
  private static storeHitboxPositionsIfValid(
    ...
    if (mag(hx!, hy!, hz!) > Arena1v1Room.HITBOX_OFFSET_MAX) return;
    if (mag(bcx!, bcy!, bcz!) > Arena1v1Room.HITBOX_OFFSET_MAX) return;
    if (mag(stx!, sty!, stz!) > Arena1v1Room.HITBOX_OFFSET_MAX) return;
    if (mag(px!, py!, pz!) > Arena1v1Room.HITBOX_OFFSET_MAX) return;
    if (mag(fx!, fy!, fz!) > Arena1v1Room.HITBOX_OFFSET_MAX) return;
  ...
  private hitscanRaycast(
    ...
    const shootPosDelta = hasShootPos
      ...
      : Infinity;
    const useShootPos =
      hasShootPos && shootPosDelta <= Arena1v1Room.CLIENT_POS_TRUST_RADIUS;
    ...
    const useClientOrigin =
      hasClientPos && clientDelta <= Arena1v1Room.CLIENT_POS_TRUST_RADIUS;
    ...
    this.state.players.forEach((p, targetId) => {
      ...
      if (process.env.DEBUG_HITSCAN && !useBoneHitboxes) {
        console.log(
          `[Arena1v1] Target ${targetId} using fallback hitboxes (missing bone data)`
        );
      }
