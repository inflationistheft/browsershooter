...
server
    ...
    .then(() => {
    console.log(`Colyseus listening on ws://${serverConfig.host}:${serverConfig.port}`);
})
    .catch((err) => {
    if (err.code === "EADDRINUSE") {
        console.error(`Port ${serverConfig.port} is already in use. Stop the other process (e.g. previous npm run dev) or use PORT=2568 npm run start:server`);
