...
- **Rampen (ramp_1x4): Kollision und Movement (Detail):**
  - **Modell: „Einfach ein Boden mit Winkel“:** Jede Rampe ist in der Kollision **ein geneigter Boden** – eine Ebene mit `getBlockTopYAt(px, pz)` (lineare Schräge entlang rampAxis). Es gibt **keine** `walkableTopOnly`-Filterung für Rampen: Sobald `(px, pz)` im Block-Footprint und in der Steigungsachse liegt, zählt die Rampe immer als Boden. So funktioniert Landen und Laufen zuverlässig; von der tiefen Seite kann man ggf. „hochgezogen“ werden (akzeptabler Trade-off).
  - **Stacken:** Rampen können übereinander platziert werden. Bei überlappenden XZ-Flächen liefert `getGroundYAt` die **höchste** Fläche, die noch unter `py + RAMP_CONTINUITY_TOLERANCE` liegt – man steht auf der oberen Rampe, beim Springen landet man darauf, dazwischen bleibt die untere.
  ...
  - **Step-up (Reinlaufen):** Beim Laufen in eine Rampe/Stufe prüft die Bewegung optional Boden etwas voraus in Bewegungsrichtung; liegt dort höherer Boden innerhalb der Step-Höhe, wird der Spieler darauf gesnappt („Hochsteigen“).
