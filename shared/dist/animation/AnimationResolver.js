...
export const DEFAULT_CLIP_NAMES = {
    idle: "idleaiming",
    walk: "walk",
...
export function resolveAnimationClipId(input) {
    ...
    if (crouching) {
        if (moveZ > INPUT_THRESHOLD)
            return ANIMATION_CLIP_IDS.crouchForward;
        if (moveZ < -INPUT_THRESHOLD)
            return ANIMATION_CLIP_IDS.crouchBackward;
        if (moveX < -INPUT_THRESHOLD)
            return ANIMATION_CLIP_IDS.crouchLeft;
        if (moveX > INPUT_THRESHOLD)
            return ANIMATION_CLIP_IDS.crouchRight;
        return ANIMATION_CLIP_IDS.crouchForward;
    }
    if (sprint && Math.abs(moveX) > INPUT_THRESHOLD) {
        if (moveX < 0)
            return ANIMATION_CLIP_IDS.strafeLeftFast;
    ...
    if (moveZ > INPUT_THRESHOLD)
        return ANIMATION_CLIP_IDS.walk;
    if (moveZ < -INPUT_THRESHOLD)
        return ANIMATION_CLIP_IDS.walkBackwards;
    if (moveX < -INPUT_THRESHOLD)
        return ANIMATION_CLIP_IDS.strafeLeftFast;
    if (moveX > INPUT_THRESHOLD)
        return ANIMATION_CLIP_IDS.strafeRightFast;
...
export function resolveAnimationClipName(input, clipNames = DEFAULT_CLIP_NAMES) {
