import type { MapData, MapPrefabPlacement } from "../types/index.js";
...
function placementToAabb(
  placement: MapPrefabPlacement,
  def: PrefabDefLike,
  rampMinYOverride?: number
): StaticBlockCollider | null {
  if (def.collision !== "box" && def.collision !== "ramp") return null;
  const [sx, sy, sz] = def.size;
  const [cx, cy, cz] = placement.position.map(Number) as [number, number, number];
  ...
  const block: StaticBlockCollider = { minX, maxX, minY, maxY, minZ, maxZ };
  if (def.collision === "ramp") {
    block.rampAxis = rot90 ? "x" : "z";
    block.rampInverted = rot === 270 || rot === 180;
  }
  if ((def as PrefabDefLike & { walkableTopOnly?: boolean }).walkableTopOnly) {
...
const RAMP_1X4_FALLBACK: PrefabDefLike = {
  collision: "ramp",
  size: [4, 1.2, 2],
...
function getRampRuns(
  placements: MapPrefabPlacement[],
  rampId: string,
  rampDepth: number
): MapPrefabPlacement[][] {
  ...
  for (const [, list] of byRot) {
    ...
    const rowKey = (p: MapPrefabPlacement) =>
      slopeAxisZ ? `${Number(p.position[0]).toFixed(2)}` : `${Number(p.position[2]).toFixed(2)}`;
    const step = (p: MapPrefabPlacement) =>
      slopeAxisZ ? Number(p.position[2]) : Number(p.position[0]);
    ...
    for (const row of byRow.values()) {
      ...
      for (let i = 1; i < row.length; i++) {
        ...
        const gap = Math.abs(
          curStep - prevStep - (slopePositive ? rampDepth : -rampDepth)
        );
        if (gap <= ADJACENT_EPS) {
          run.push(cur);
        } else {
...
export function buildKillVolumesFromMap(
  map: MapData,
  prefabDefs: PrefabDefsLike
): StaticBlockCollider[] {
  const volumes: StaticBlockCollider[] = [];
  for (const placement of map.prefabs) {
    if (placement.id !== "kill_volume") continue;
...
export function buildStaticWorldFromMap(
  ...
  for (const run of rampRuns) {
    ...
    for (const placement of run) {
      ...
      const aabb = placementToAabb(
        placement,
        def as PrefabDefLike,
        prevMaxY
      );
      if (aabb) {
        blocks.push(aabb);
...
function getBlockTopYAt(b: StaticBlockCollider, px: number, pz: number): number {
  if (!b.rampAxis) return b.maxY;
...
function isInsideRampAxis(b: StaticBlockCollider, px: number, pz: number): boolean {
  if (!b.rampAxis) return true;
  const axisMin = b.rampAxis === "z" ? b.minZ : b.minX;
  const axisMax = b.rampAxis === "z" ? b.maxZ : b.maxX;
  const p = b.rampAxis === "z" ? pz : px;
...
export function isOnRamp(
  ...
  const pairs: { b: StaticBlockCollider; topY: number }[] = [];
  for (const b of world.blocks) {
    if (
      px < b.minX - margin ||
      px > b.maxX + margin ||
...
export function resolveStaticWorldXZ(
  px: number,
  py: number,
  pz: number,
  radius: number,
...
export function rayStaticWorldIntersection(
  ox: number,
  oy: number,
  oz: number,
  dx: number,
