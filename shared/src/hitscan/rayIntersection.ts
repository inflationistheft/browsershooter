 ...
 */
export function rayCapsuleIntersection(
  ox: number,
  oy: number,
  oz: number,
  dx: number,
  ...
  const tCyl = intersectRayCylinder(
    ox, oy, oz, dx, dy, dz,
    ax, ay, az, bax, bay, baz, baLenSq,
    oaDotBa, dDotBa,
    r
  );
  if (tCyl !== null && (bestT === null || tCyl < bestT)) bestT = tCyl;
...
function intersectRayCylinder(
  ...
  bax: number, bay: number, baz: number,
  baLenSq: number,
  oaDotBa: number, dDotBa: number,
  r: number
): number | null {
  const oax = ox - ax;
  const oay = oy - ay;
  ...
  if (t2 > 0 && (t === null || t2 < t)) {
    ...
    if (s >= 0 && s <= 1) t = t === null ? t2 : Math.min(t, t2);
  }
  return t;
}
